...
public class ReportRecordController {
	 ...
	 * @return
	 */
	@RequestMapping(value = {"updateBreakdown"})
	@ResponseBody
	public ReturnEntity<String> updateBreakdown(HttpServletRequest request,WtStationBreakdown breakdown){
		ReturnEntity<String> re = new ReturnEntity<String>();
		reportRecordServiceImpl.updateBreakdown(breakdown);
		return re;
	}
	...
	@ResponseBody
	public ReturnEntity<String> updateRepair(HttpServletRequest request,WtStationRepair repair){
		ReturnEntity<String> re = new ReturnEntity<String>();
		String repairId = request.getParameter("repairId");//这个就是repaire的主键id
		repair.setId(Integer.valueOf(repairId));
		reportRecordServiceImpl.updateRepair(repair);
		return re;
	}
